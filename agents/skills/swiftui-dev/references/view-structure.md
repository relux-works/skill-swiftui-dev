...
Each type (struct, class, enum) should be in its own Swift file. Flag files containing multiple type definitions. This keeps files focused, improves navigation, and aligns with the subview extraction pattern below.
...
Strongly prefer to avoid breaking up view bodies using computed properties or methods that return `some View`, even if `@ViewBuilder` is used. Extract them into separate `View` structs instead, placing each into its own file. This rule is critical enough to be stated twice -- it is the single most impactful pattern for SwiftUI performance and maintainability.
...
Button actions should be extracted from view bodies into separate methods, to avoid mixing layout and logic. If a button action can be provided directly as an `action` parameter, do so:

```swift
// Good - action as parameter
Button("Save", action: saveItem)
Button("Add", systemImage: "plus", action: addItem)

// Avoid - inline logic in closure
Button("Save") {
    isLoading = true
    apiService.save(item) { result in
...
General business logic should not live inline in `task()`, `onAppear()` or elsewhere in `body`. Place view logic into view models or similar so it can be tested.
...
Unless a full-screen editing experience is required, prefer `TextField` with `axis: .vertical` over `TextEditor` -- it supports placeholder text:

```swift
// Good - placeholder text, auto-grows
TextField("Enter description", text: $description, axis: .vertical)
    .lineLimit(5...)

// Avoid unless full-screen editing is needed
...
When using `TabView(selection:)`, bind to an enum, not integer or string:
...
- Use the `@Animatable` macro over creating `animatableData` manually. The macro automatically adds conformance to the `Animatable` protocol and creates the correct `animatableData` property. Mark non-animatable properties (Booleans, integers, etc.) with `@AnimatableIgnored`.
- Never use `animation(_ animation: Animation?)` without a value parameter. Always provide a value to watch: `.animation(.bouncy, value: score)`.
- Chain animations using `completion` closures in `withAnimation()`, not multiple `withAnimation()` calls with delays:
...
- [ ] Complex views extracted to separate subview structs (not computed properties)
- [ ] Each extracted subview in its own file
- [ ] Views kept small for better performance
- [ ] `@ViewBuilder` functions only for simple sections
- [ ] Button actions extracted into methods, not inline closures
- [ ] Business logic in view models, not inline in `body`/`task()`/`onAppear()`
- [ ] `TextField(axis: .vertical)` over `TextEditor` unless full-screen editing needed
- [ ] `ImageRenderer` over `UIGraphicsImageRenderer` for view-to-image
- [ ] `#Preview` over legacy `PreviewProvider`
- [ ] `TabView(selection:)` bound to enum, not integer/string
- [ ] `@Animatable` macro over manual `animatableData`
- [ ] `.animation()` always includes a `value` parameter
- [ ] Chain animations via `withAnimation` completion closures
