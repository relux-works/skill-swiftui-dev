...
Keep SwiftUI body code and logic code separate. Place logic into methods or `@Observable` classes, not inline in `body`.
...
- Strongly prefer to avoid creating bindings using `Binding(get:set:)` in view body code. Use a binding from `@State`, `@Binding` etc, then use `onChange()` to trigger effects.
- If the user needs to enter a number into a `TextField`, bind to a numeric value with `format`: `TextField("Enter your score", value: $score, format: .number)`. Apply `.keyboardType(.numberPad)` or `.keyboardType(.decimalPad)` as appropriate. The modifier alone is NOT sufficient.
...
- Never use `@AppStorage` inside an `@Observable` class, even if marked `@ObservationIgnored` — it will NOT trigger view updates.
- Use `@State` as a cache for expensive non-observable objects (e.g. `CIContext`) that need to persist across view updates.
...
9. **Keep body code and logic separate** — place logic into methods or `@Observable` classes

## SwiftData with CloudKit

When using SwiftData with CloudKit sync, follow these constraints strictly:
